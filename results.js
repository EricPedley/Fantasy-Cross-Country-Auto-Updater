    ...
    for (let i = 0; i != -1; i = html.indexOf('"IDMeetDiv":', i + 1)) {
      ...
      if (Number(id)) {
        ...
        rp(raceLink)
          .then(function (html) {
            let totalRunners=0;
            for (let i = 0; i != -1; i = html.indexOf('"Place":', i + 1)) {
              let athleteIDIndex = html.indexOf('"AthleteID":',i);
              let athleteID = html.substring(athleteIDIndex+12,athleteIDIndex+20);
              if(athleteID===-1)
                break;
              if(Number(athleteID))
                totalRunners++;
              if(athleteIDs[athleteID]) {
                console.log(athleteIDs[athleteID]);
              }
            }
